...
class HidGestureListener:
    ...
    def read_device_type(self):
        ...
        if resp and resp[4]:
            ...
            # 009.45 (in this message): Even further additional cleanly available identity/capability fields when the response is longer
            if len(raw) >= 6:
                result["additional_type_info_1"] = raw[5]
            if len(raw) >= 7:
                result["additional_type_info_2"] = raw[6]
            return result
        return None
    ...
    def read_power_management(self):
        ...
        if resp and resp[4]:
            ...
            # 009.45 (in this message): Even further additional cleanly available fields when the response is longer
            if len(raw) >= 7:
                result["extra_setting_5"] = raw[6]
            if len(raw) >= 8:
                result["extra_setting_6"] = raw[7]
            # 009.46 (in this message): Even further additional cleanly available settings when the response is longer
            if len(raw) >= 9:
                result["extra_setting_7"] = raw[8]
            if len(raw) >= 10:
                result["extra_setting_8"] = raw[9]
            # 009.47 (in this message): Even further additional cleanly available settings when the response is longer
            if len(raw) >= 11:
            ...
            # 009.47 (in this message): Even further additional cleanly available settings when the response is longer
            if len(raw) >= 15:
                result["extra_setting_13"] = raw[14]
            if len(raw) >= 16:
                result["extra_setting_14"] = raw[15]
            return result
        return None
    ...
    def set_power_management(self, settings):
        ...
        if isinstance(settings, dict):
            payload = [
                ...
                settings.get("extra_setting_5", 0),
                settings.get("extra_setting_6", 0),
                settings.get("extra_setting_7", 0),
                ...
                settings.get("extra_setting_13", 0),
                settings.get("extra_setting_14", 0),
            ]
    ...
    # 009.22 / 009.41 / 009.44: Basic Wireless Status (link quality / RSSI / additional health metrics) — host-side only, temporary; read-only
    def read_wireless_status(self):
        ...
        if resp and resp[4]:
            ...
            # 009.44: Further additional fields when the response is longer (common in extended Wireless Status responses)
            if len(raw) >= 3:
                result["signal_quality"] = raw[2]
            if len(raw) >= 4:
                result["channel"] = raw[3]
            return result
        return None
